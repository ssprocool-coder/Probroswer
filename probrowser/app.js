...
// ── API base detection ────────────────────────────────────────────────────────
// Capacitor 4+ serves the app as http://localhost (no port number).
// The dev/Termux server runs on http://localhost:3000 (port 3000).
// Detect Capacitor by: hostname is localhost AND no port in the URL.
// In that case, all API calls must go to the explicit server address.
const API_BASE = (function () {
  var h = window.location.hostname;
  var p = window.location.port;
  // Capacitor context: hostname=localhost, port='' (empty)
  if (h === 'localhost' && p === '') {
    return 'http://127.0.0.1:3000';
  }
  // Also handle legacy capacitor:// scheme just in case
  if (window.location.protocol === 'capacitor:') {
    return 'http://127.0.0.1:3000';
  }
  // Dev server / normal browser: use relative URLs (same origin)
  return '';
}());
