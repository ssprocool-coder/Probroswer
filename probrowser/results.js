...
// ── API base detection ────────────────────────────────────────────────────────
// Capacitor 4+ serves the app as http://localhost (hostname=localhost, port='').
// The Termux/Node server runs on http://localhost:3000 (port='3000').
// When running inside Capacitor: port is empty, so we must call the server explicitly.
// When running in dev browser at localhost:3000: port is '3000', use relative URLs.
const API_BASE = (function () {
  var h = window.location.hostname;
  var p = window.location.port;
  // Capacitor context: hostname=localhost, no port
  if (h === 'localhost' && p === '') {
    return 'http://127.0.0.1:3000';
  }
  // Also handle legacy capacitor:// scheme
  if (window.location.protocol === 'capacitor:') {
    return 'http://127.0.0.1:3000';
  }
  // Dev server / normal browser (localhost:3000): relative URLs work fine
  return '';
}());
...
async function loadAIOverview() {
  ...
  try {
    const r = await fetch(`${API_BASE}/api/ai/overview?q=${encodeURIComponent(currentQuery)}`);
    const data = await r.json();
    if (data.unavailable) {
...
async function loadImages() {
  try {
    const r = await fetch(`${API_BASE}/api/search/images?q=${encodeURIComponent(currentQuery)}`);
    const data = await r.json();
...
async function loadVideos() {
  try {
    const r = await fetch(`${API_BASE}/api/search/videos?q=${encodeURIComponent(currentQuery)}`);
    const data = await r.json();
    if (!data.results?.length) {
